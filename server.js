            ...
            if(value) {
                ...
                if(typeof filterString === "string") {
                    for(let i = 0; i < value.length; i++) {
                        if(value["name"].toLowerCase().includes(filterString.toLowerCase())) {
    ...
    if(!reqBody["name"] || !reqBody["duration"]) {
        ...
        if(!cookieHeader) {
            ...
            if(!sessionManager.getUsername(parsedCookieHeader[1])) {
                res.status(404);
                res.send("Could not find username with cookie: " + parsedCookieHeader[1]);
            }
            else {
                const task = {};
                ...
                task["finished"] = false;
                task["author"] = sessionManager.getUsername(parsedCookieHeader[1]).username;
    ...
    db.getTask(req.params["task_id"]).then(value => {
        if(value) {
            ...
            if(!sessionManager.getUsername(parsedCookieHeader)) {
                ...
                const username = sessionManager.getUsername(parsedCookieHeader).username;
                if(value["author"] === username) {
                    ...
                    res.status(400);
                    res.send(username + " is not the author of this task, cannot be deleted");
                }
            }
        }
        else {
    ...
    db.getTask(req.params["task_id"]).then(value => {
        if(value) {
            ...
            if(!sessionManager.getUsername(parsedCookieHeader)) {
                ...
                if(value["author"] === username) {
                    const reqBody = JSON.parse(JSON.stringify(req.body));
                    if('name' in reqBody || 'duration' in reqBody || 'finished' in reqBody) {
                        ...
                        res.status(404);
                        res.send("Could not find name or duration or finished key in object");
                    }
                }
                else {
                    res.status(400);
                    res.send(username + " is not the author of this task, cannot be updated");
                }
            }
        }
        else {
...
function isCorrectPassword(password, saltedHash) {
    ...
    const shaHash = crypto.createHash('sha256').update(saltedPassword).digest('base64');
